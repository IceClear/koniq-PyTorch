...
def train_model(model,optimizer,batch_size, num_epochs=40):
    ids = pd.read_csv('../koniq10k/koniq10k_distributions_sets.csv')
    data_dir='../koniq10k/1024x768/'
...
def rating_metrics(y_true, y_pred, show_plot=False):
    """
...
def test():
    ### Test model on the default test set, pd bug
    KonCept512 = model_qa(num_classes=1)
    KonCept512.load_state_dict(torch.load('../koniq10k/KonCept512.pth'))
    KonCept512.eval().to(device)

    ids = pd.read_csv('../koniq10k/koniq10k_distributions_sets.csv', error_bad_lines=False)
    data_dir='../koniq10k/1024x768/'
...
def calculate_score():
    ...
    img = data_transforms['val'](Image.open(os.path.join('/mnt/lustre/jywang/code/IL-NIQE/pepper_exa/pepper_3.png')))
    img = img.unsqueeze(0).to(device)
    output = KonCept512(img).detach().cpu().numpy()
